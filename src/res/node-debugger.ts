...
export class NodeDebugger extends EventEmitter {
    ...
    public connect(port: number): Promise<void> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.ws.on('open', () => {
                    this.ws!.send(JSON.stringify({ id: this.getRequestId(), method: "Debugger.enable" }));
                    this.ws!.send(JSON.stringify({ id: this.getRequestId(), method: "Runtime.enable" }));
                });
                this.ws.on('message', (data) => {
                    const dataObj = JSON.parse(data.toString());
                    if (dataObj.method === "Runtime.executionContextCreated" && isNaN(this.executionContextId)) {
                        this.executionContextId = dataObj.params.context.id;
                        resolve();
                        return;
                    }
                    if (dataObj.method === "Runtime.executionContextDestroyed" && this.executionContextId === dataObj.params.executionContextId) {
                        this.emit('stopped');
                        return;
                    }
    ...
    public evaluate(callFrameId: string, expression: string): Promise<any> {
        return this.callProtocol("Debugger.evaluateOnCallFrame", { callFrameId, expression, returnByValue: true });
    }
