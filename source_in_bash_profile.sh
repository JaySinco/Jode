...
_build_completions()
{
    if [ -d "${CMAKE_OUT_DIR}" ]; then
        COMPREPLY+=($(compgen -W "$(find ${CMAKE_OUT_DIR} -maxdepth 1 -name *.vcxproj | grep -v [[:upper:]] | awk -F [/.] '{print $2}')" -- "${COMP_WORDS[-1]}"))
